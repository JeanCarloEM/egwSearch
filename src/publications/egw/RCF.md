...
A raiz local e `src/publications/`; cada grupo DEVE ocupar `egw/<language>/<type>/<slug-titulo>/`. PDF, EPUB e metadado local do grupo DEVEM usar o mesmo acronimo, respectivamente `<acronimo>.pdf`, `<acronimo>.epub` e `<acronimo>.source.json`. [c5c4da6]
...
Testes sem rede DEVEM cobrir normalizacao editorial, transliteracao e slug RFC 3986, acronimo, path, assinatura, hashes, colisao de slug, metadado legado/v2, plano, repeticao, apply, remocao de diretorio vazio, rollback e falha de persistencia entre intencao e confirmacao. [c5c4da6]
