...
Checkpoint ausente inicia nova execução; checkpoint incompatível, ambíguo ou corrompido DEVE ser preservado em quarentena auditável e reconstruído automaticamente a partir de publicações, provas e prefixos locais inequivocamente válidos, sem apagar publicação canônica já promovida nem repetir rede evitável. [PENDENTE-CODIGO]
`--restart` permanece como reinício humano explícito limitado ao escopo selecionado e DEVE descartar apenas checkpoints de runtime aplicáveis; sua existência NÃO PODE substituir a autorrecuperação de estado tecnicamente reparável. [PENDENTE-CODIGO]
...
Alteração preexistente que pertença a outra publicação e não seja artefato global causal da unidade corrente NÃO DEVE bloquear preflight, materialização ou fechamento: ela DEVE permanecer visível e intocada na worktree, ser excluída da allowlist, do staging e do commit corrente e continuar pertencendo ao fluxo que a produziu; estado parcial da própria unidade DEVE ser copiado para recuperação auditável e retomado/recalculado localmente, enquanto artefato global derivado divergente DEVE ser reconstruído deterministicamente. [PENDENTE-CODIGO]

O comando canônico de materialização offline DEVE consumir exclusivamente checkpoints, arquivos e provas locais já persistidos, aceitando coleção integralmente descoberta ou escopo explícito inequivocamente contido no prefixo enriquecido de descoberta parcial, sem inferir completude da coleção; NÃO PODE inicializar navegador, cliente HTTP, resolução DNS ou requisição. Entrada ausente ou insuficiente DEVE permanecer pendente e diagnosticada sem impedir unidades independentes posteriores, sem apagar checkpoint e sem confirmar item ou coleção incompletos. [PENDENTE-CODIGO]

Downloader, materializador offline e indexador DEVEM adquirir a mesma exclusão mútua de processo antes de ler ou escrever diário, checkpoint, publicação, análise ou índice compartilhado; o lock DEVE ser mantido pelo sistema operacional e liberado automaticamente no encerramento, de modo que concorrência falhe antes de qualquer efeito e arquivo residual sem lock ativo não bloqueie retomada. [PENDENTE-CODIGO]

Antes de qualquer escrita, o downloader DEVE comparar a identidade remota com a ocupação atual e com a última unidade Git íntegra do destino; IDs distintos que resultem na mesma rota editorial NÃO PODEM se sobrescrever, DEVEM conservar título e autoria originais e receber discriminador físico estável derivado da identidade remota. Sobrescrita já ocorrida e inequivocamente comprovada DEVE ser copiada integralmente para runtime recuperável, ter a unidade rastreada restaurada e refazer somente a identidade colidente no destino desambiguado. [PENDENTE-CODIGO]

Downloader e indexador DEVEM ser autorresolutivos para pendência, corrupção, parcialidade ou divergência tecnicamente reparável: preservar bytes divergentes, reutilizar prova válida, recalcular análise afetada, reconstruir índice incompatível/duplicado e validar novamente antes de promover ou confirmar. É VEDADO encerrar apenas com orientação de correção manual quando exista reparo local seguro e determinístico; intervenção humana somente PODE ser exigida depois de esgotado o reparo cirúrgico sem perda e comprovada a irresolubilidade material. [PENDENTE-CODIGO]
...
Nova invocação global compatível DEVE retomar automaticamente do limite confirmado sem repetir publicação, ativo, experimento, indexação ou commit; parâmetro explícito de reset, propagado por wrappers e composições, PODE descartar o cursor estritamente pertencente ao escopo solicitado, enquanto reparo automático DEVE preservá-lo em quarentena antes de reconstruir somente o estado inválido. [PENDENTE-CODIGO]

Cursor ausente DEVE iniciar o escopo; cursor corrompido, futuro ou incompatível DEVE ser preservado em quarentena e reconstruído automaticamente quando configuração, corpus, catálogo e versão permitirem prova determinística, e somente ambiguidade material sem base íntegra PODE exigir intervenção humana. O diário permanece fora de Git, publicação, índice, build e Pages. [PENDENTE-CODIGO]
